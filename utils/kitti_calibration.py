...
class Calibration(object):
    ...
    def lidar_to_cam(self, pts_lidar):
        ...
        :param pts_cam: np.array (4, 1)
        :return: pts_lidar: np.array (4, 1)
        """
        return np.dot(self.V2C, pts_lidar)
    ...
    def rect_to_img(self, pts_rect):
        ...
        # print('pts_2d_hom:', pts_2d_hom.shape)
...
if __name__ == '__main__':
    import cv2
    calib = Calibration('/data/semanticKITTI/sequences/00/calib.txt')
    o3pts = open3d.io.read_point_cloud('/data/semanticKITTI/sequences/00/processed_pcd/000000.pcd')
    points = np.asarray(o3pts.points, dtype=np.float32)
    print(points.shape)
    out, _ = calib.lidar_to_img(points)
    print(out.shape)
    img = cv2.imread('/data/semanticKITTI/sequences/00/image_2/000000.png')
    for p in out:
        # print(p)
        x = np.floor(p[0])
        y = np.floor(p[1])
        if 0 < x < 1241 and 0 < y < 376:
            image = cv2.circle(img, (int(x), int(y)), 0, (255, 0, 0), -1)
    cv2.imshow("1", image)
    cv2.waitKey()